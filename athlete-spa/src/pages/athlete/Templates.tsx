...
export default function AthleteTemplatesPage() {
  ...
  // Fetch Plans (Created by athlete)
  const { data: myPlans, isLoading: myPlansLoading, refetch: refetchMyPlans } = useCollection<TrainingPlan>(
    'workoutPlans',
    user?.uid ? { trainerId: user.uid } : undefined
  );

  // Fetch Plans (Assigned to athlete)
  ...
  const handleCreatePlan = async (data: TrainingPlan) => {
    try {
      // When athlete creates a plan, they are the trainerId
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {workoutsLoading ? <p>Ładowanie...</p> : workouts?.map(workout => (
              <Card key={workout.id}>
                <CardHeader>
                  ...
                  <CardDescription>{workout.durationMinutes} min • {workout.exerciseSeries.length} ćwiczeń</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            {!workoutsLoading && workouts?.length === 0 && (
              <div className="col-span-full text-center py-12 text-muted-foreground border-2 border-dashed rounded-lg">
                ...
                <p>Brak własnych treningów. Stwórz pierwszy trening!</p>
              </div>
            )}
          </div>
        </TabsContent>
