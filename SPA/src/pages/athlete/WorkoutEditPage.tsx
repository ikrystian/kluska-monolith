...
export default function WorkoutEditPage() {
  ...
  const { data: workout, isLoading: workoutLoading } = useDoc<Workout>('workouts', id ?? null);
  const { data: allExercises, isLoading: exercisesLoading } = useCollection<Exercise>(
    'exercises',
    { limit: 200 }
  );
  const { mutate: updateWorkout, isPending: isSaving } = useUpdateDoc<Workout>('workouts');
  ...
  useEffect(() => {
    if (workout && !isInitialized) {
      ...
      if (workout.exerciseSeries) {
        const workoutExercises = workout.exerciseSeries as unknown as { exerciseId?: string; name?: string; sets?: number; reps?: number; restSeconds?: number }[];
        setExercises(workoutExercises.map(ex => ({
  ...
  const handleSave = () => {
    ...
    updateWorkout({ id, data: workoutData }, {
      onSuccess: () => {
        toast.success('Trening zaktualizowany!');
        navigate(`/athlete/workouts/${id}`);
      },
      onError: () => toast.error('Nie udało się zaktualizować treningu'),
    });
  };
  ...
  if (!workout) {
    return (
      <div className="container mx-auto p-4 md:p-8 text-center">
        <h1 className="text-2xl font-bold mb-4">Trening nie znaleziony</h1>
        <Button onClick={() => navigate('/athlete/workouts')}>
          Powrót do listy
        </Button>
      </div>
    );
  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      {/* Header */}
      <div className="mb-6">
        <Button variant="ghost" onClick={() => navigate(`/athlete/workouts/${id}`)} className="mb-4 pl-0">
          <ArrowLeft className="mr-2 h-4 w-4" />
          Powrót do treningu
        </Button>
        <h1 className="font-headline text-3xl font-bold">Edytuj Trening</h1>
        <p className="text-muted-foreground">Modyfikuj szczegóły treningu.</p>
      </div>
