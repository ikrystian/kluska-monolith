...
import type { WorkoutPlan, UserProfile, TrainingPlan } from '@/lib/types';
import {
  Table,
  TableBody,
  TableCell,
...
export default function AdminWorkoutPlansPage() {
  ...
  // Using TrainingPlan type now
  const { data: plans, isLoading: plansLoading, refetch: refetchPlans } = useCollection<TrainingPlan>('workoutPlans');
    ...
    <div className="container mx-auto p-4 md:p-8">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button asChild>
            <Link href="/admin/workout-plans/new">
              ...
              {isLoading ? (
                ...
                plans.map((plan: TrainingPlan) => (
                  <TableRow key={plan.id}>
                    ...
                    <TableCell>{usersMap.get(plan.trainerId) || (plan.trainerId ? 'Trener' : 'System')}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" asChild>
                          <Link href={`/admin/workout-plans/${plan.id}`}>
