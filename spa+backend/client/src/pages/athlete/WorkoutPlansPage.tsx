...
function WorkoutPlanCard({ plan }: { plan: TrainingPlan }) {
    const totalWeeks = plan.stages?.reduce((acc, stage) => acc + (stage.weeks?.length || 0), 0) || 0;
    const totalWorkouts = plan.stages?.reduce((acc, stage) =>
        acc + (stage.weeks?.reduce((weekAcc, week) =>
            weekAcc + (week.days?.filter(day => day !== 'Rest Day' && day !== null).length || 0), 0) || 0
        ), 0) || 0;
            ...
            <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4 text-center">
                    <div className="space-y-1">
                        ...
                        <p className="text-lg font-bold">{plan.stages?.length || 0}</p>
                        ...
                        <AccordionItem value={`stage-${stageIndex}`} key={stageIndex}>
                            <AccordionTrigger className="hover:no-underline">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-muted-foreground text-sm ml-2">({stage.weeks?.length || 0} tyg.)</span>
                                </div>
                            </AccordionTrigger>
                            <AccordionContent>
        ...
        <div className="container mx-auto p-4 md:p-8 space-y-12">
            ...
            <section id="trainers" className="space-y-6">
                <div className="flex flex-col md:flex-row justify-between items-end gap-4">
                    <div>
                        <h2 className="text-3xl font-bold">Nasi Trenerzy</h2>
                        <p className="text-muted-foreground mt-2">
                                            ...
                                            <CardContent className="text-center space-y-4">
                                                ...
                                                <div className="pt-4">
                                                    <Button className="w-full rounded-full group-hover:bg-primary group-hover:text-primary-foreground transition-colors" asChild>
                                                        <Link to={`/athlete/chat?userId=${trainer.id}`}>
                                                            <MessageSquare className="mr-2 h-4 w-4" />
                                                            Nawiąż Współpracę
                                                        </Link>
                                                    </Button>
