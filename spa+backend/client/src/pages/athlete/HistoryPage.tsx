...
export default function HistoryPage() {
    ...
    const handleDelete = async (sessionId: string) => {
        ...
        try {
            await api.deleteDocument('workoutLogs', sessionId);
            toast({
    ...
    return (
        <AlertDialog>
            <div className="container mx-auto p-4 md:p-8">
                <h1 className="mb-6 text-3xl font-bold">Historia Treningów</h1>
                <Card>
                    <CardHeader>
                        <CardTitle>Twój Dziennik</CardTitle>
                                        ...
                                        <div className="flex items-center">
                                            <AccordionTrigger className="hover:no-underline flex-grow" onClick={() => navigate(`/athlete/history/${log.id}`)}>
                                                <div className="flex w-full items-center justify-between pr-4">
                                                    <div className="text-left">
                                                        ...
                                                        <p className="text-sm text-muted-foreground">{log.endTime ? format(new Date(log.endTime), 'd MMMM yyyy', { locale: pl }) : '-'}</p>
                                                    </div>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    ...
                                                    <AlertDialogDescription>
                                                        Tej operacji nie można cofnąć. To spowoduje trwałe usunięcie treningu "{log.workoutName}" z dnia {log.endTime ? format(new Date(log.endTime), 'd.MM.yyyy') : '-'}.
                                                    </AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
