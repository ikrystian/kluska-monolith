...
function ActiveWorkoutView({ initialWorkout, allExercises, onFinishWorkout, isLoadingExercises, initialLogId, sourceWorkoutId }: { initialWorkout: LogFormValues; allExercises: Exercise[] | null; onFinishWorkout: () => void; isLoadingExercises: boolean; initialLogId?: string | null; sourceWorkoutId?: string | null }) {
  ...
  useEffect(() => {
    ...
    const createInitialWorkoutLog = async () => {
      ...
      try {
        const result = await api.createDocument<{ data: WorkoutLog }>('workoutLogs', initialLogData);
  ...
  useEffect(() => {
    if (workoutLogId) {
      ...
      if (currentLogId !== workoutLogId) {
        ...
        navigate(`/athlete/log?logId=${workoutLogId}`, { replace: true });
      }
    }
  ...
  useEffect(() => {
    ...
    const saveTimeout = setTimeout(async () => {
      ...
      try {
        await api.updateDocument('workoutLogs', workoutLogId, updateData);
  ...
  const handleSaveWorkout = async (data: LogFormValues) => {
    ...
    if (photoFile) {
      try {
        ...
        const uploadResult = await api.request<{ url: string }>('/upload', {
          method: 'POST',
          body: formData,
        });
    ...
    try {
      await api.updateDocument('workoutLogs', workoutLogId, finalLogData);
  ...
  const handleDiscardWorkout = async () => {
    ...
    setIsSaving(true);
    try {
      await api.deleteDocument('workoutLogs', workoutLogId);
          ...
          <div className="pt-4 space-y-4">
            {photoPreview && (
              <div className="relative w-full aspect-video rounded-md overflow-hidden mb-2">
                <img src={photoPreview} alt="Podgląd zdjęcia" className="w-full h-full object-cover" />
              </div>
            )}
