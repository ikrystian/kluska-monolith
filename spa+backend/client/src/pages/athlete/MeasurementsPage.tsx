<<<<<<< HEAD:SPA/src/pages/athlete/MeasurementsPage.tsx
<<<<<<< HEAD
...
>>>>>>> 3f020ae (chore: Update project dependencies and remove the `ChatPage.tsx` file.):spa+backend/client/src/pages/athlete/MeasurementsPage.tsx
...
=======
  ...
  const onSubmit = async (data: MeasurementFormValues) => {
    ...
    if (photosToUpload.length > 0) {
      try {
        setIsUploading(true);
        photoURLs = await uploadPhotos(photosToUpload);
      } catch (error) {
                        ...
                        <div className="flex -space-x-2 overflow-hidden">
                          {session.photoURLs.slice(0, 3).map((url, i) => (
                            <div key={i} className="relative h-8 w-8 rounded-full border-2 border-background overflow-hidden">
                              <img src={url} alt="Miniaturka" className="w-full h-full object-cover rounded-full" />
                            </div>
                          ))}
