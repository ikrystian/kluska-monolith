...
import { format, addDays, startOfMonth, endOfMonth, eachDayOfInterval, isSameDay, getMonth, getYear } from 'date-fns';
...
export function ScheduleWorkoutDialog({
    ...
    // For simplicity and "infinite" feel, let's generate a range around the selected date or just next 30 days?
    // The image shows "December" and a list of days. Let's show the current month of the selected date.
    ...
    }, [open, currentMonthDate]); // Re-run when month changes too
    ...
    const handleSchedule = async () => {
        ...
        try {
            ...
            await createDoc('plannedWorkouts', plannedWorkout);

            toast({
                title: 'Sukces',
                description: 'Trening został zaplanowany',
            });
            onOpenChange(false);
        } catch (error) {
            ...
            toast({
                title: 'Błąd',
                description: 'Nie udało się zaplanować treningu',
                variant: 'destructive',
            });
        }
    };
            ...
            <DialogContent className="sm:max-w-[425px] bg-[#09090b] text-white border-zinc-800 p-0 overflow-hidden gap-0">
                ...
                <div className="px-6 py-4 flex items-center justify-center gap-4">
                    <div className="relative">
                        ...
                        />
                        {/* Blue indicator line/dot simulation if needed, but input border is cleaner */}
                    </div>
